...
public class GraphGen {
	...
	if ( args.length != 1 && args.length != 2 ) {
	    System.out.println( "usage: java GraphGen #nodes filename" );
	    System.exit( -1 );
	}
	...
	try {
	    PrintWriter file = ( args.length == 2 ) ? new PrintWriter( args[1] ) : null;

	    System.out.println( "nNodes = " + nNodes +"\n" );
	    GraphMap[] node = new GraphMap[nNodes];
	    for ( int i = 0; i < nNodes; i++ ) {
		System.out.println( "Node " + i + ":" );
		node[i] = new GraphMap( nNodes, i );
